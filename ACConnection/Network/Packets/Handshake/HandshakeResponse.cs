...
public class HandshakeResponse : INetworkPacket
{
  ...
  public override string ToString( )
  {
    ...
    if( ChecksumPaths != null )
      foreach( var p in ChecksumPaths )
        paths.AppendLine( p.ToString( ) );
    ...
    return $"""
            ...
            CarSkin: {CarSkin}
            SunAngle: {SunAngle}
            AllowedTyresOutCount: {AllowedTyresOutCount}
            AllowTyreBlankets: {AllowTyreBlankets}
            TractionControlAllowed: {TractionControlAllowed}
            ABSAllowed: {ABSAllowed}
            StabilityAllowed: {StabilityAllowed}
            AutoClutchAllowed: {AutoClutchAllowed}
            JumpStartPenaltyMode: {JumpStartPenaltyMode}
            MechanicalDamageRate: {MechanicalDamageRate}
            FuelConsumptionRate: {FuelConsumptionRate}
            TyreConsumptionRate: {TyreConsumptionRate}
            IsVirtualMirrorForced: {IsVirtualMirrorForced}
            MaxContactsPerKm: {MaxContactsPerKm}
            RaceOverTime: {RaceOverTime}
            ResultScreenTime: {ResultScreenTime}
            HasExtraLap: {HasExtraLap}
            IsGasPenaltyDisabled: {IsGasPenaltyDisabled}
            PitWindowStart: {PitWindowStart}
            PitWindowEnd: {PitWindowEnd}
            InvertedGridPositions: {InvertedGridPositions}
