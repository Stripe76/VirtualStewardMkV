...
public struct CSPWeatherUpdate : INetworkPacket
{
  ...
  {
    return ACServerProtocol.WeatherUpdate;
  }

  public readonly override string ToString( )
  {
    return $"""
              {nameof( UnixTimestamp )}: {UnixTimestamp},
              {nameof( WeatherType )}: {WeatherType},
              {nameof( UpcomingWeatherType )}: {UpcomingWeatherType},
              {nameof( TransitionValue )}: {TransitionValue},
              {nameof( TemperatureAmbient )}: {TemperatureAmbient},
              {nameof( TemperatureRoad )}: {TemperatureRoad},
              {nameof( TrackGrip )}: {TrackGrip},
              {nameof( WindDirectionDeg )}: {WindDirectionDeg},
              {nameof( WindSpeed )}: {WindSpeed},
              {nameof( Humidity )}: {Humidity},
              {nameof( Pressure )}: {Pressure},
              {nameof( RainIntensity )}: {RainIntensity},
              {nameof( RainWetness )}: {RainWetness},
              {nameof( RainWater )}: {RainWater}
            """;
  }
